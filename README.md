#### ClueWell Mail
Phishing Email Detection & Analysis System
ClueWell Mail is a client-side cybersecurity tool that analyzes emails and classifies them as Phishing, Spam, or Legitimate using JavaScript-based rules and risk scoring.

## 🌐 Live Demo

[Click here to open ClueWell Mail](https://cluewell-mail.netlify.app/)

## ✨ Features

Phishing, spam & legitimate email detection
Sender, subject & body analysis
Suspicious URL detection
Keyword & pattern analysis
Risk and confidence scoring
Detection reasons
PDF analysis report generation
Responsive dark-themed interface

## ⚙️ How It Works
Email Input → Rule Analysis → URL & Keyword Detection → Risk Scoring → Verdict

## 📌 Important
ClueWell Mail uses a JavaScript-based rule detection engine. It does not use machine learning or an AI model.
## 👨‍💻 Author
Vansh Arora
B.Tech CSE — Cybersecurity
